...
async function setupDatabase() {
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS sde.current_weather (
      ...
      locationid INTEGER REFERENCES sde.weather_locations(locationid),
...
async function getLocationsFromDatabase(client) {
  const result = await client.query(
    'SELECT locationid, latitude, longitude FROM sde.openmeteolocations_wgs84 ORDER BY locationid'
  );
  return result.rows;
}
...
async function insertCurrentWeather(client, locationId, weatherData) {
  ...
  await client.query(
    `INSERT INTO sde.current_weather 
     (locationid, time, temperature_2m, relative_humidity_2m, precipitation, rain, wind_speed_10m)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     ON CONFLICT (locationid, time) 
     DO UPDATE SET 
  ...
  for (let i = 0; i < hourlyData.time.length; i++) {
    await client.query(
      `INSERT INTO sde.hourly_weather 
       (locationid, time, temperature_2m, relative_humidity_2m, wind_speed_10m, wind_direction_10m, precipitation, rain)
...
async function runWeatherSync() {
  ...
  console.log(`\n==== WEATHER SYNC STARTED AT ${startTime.toISOString()} ====`);
  ...
  try {
    ...
    const locations = await getLocationsFromDatabase(client);
    if (locations.length === 0) {
      throw new Error('No locations found in database. Please ensure OpenMeteoLocations_WGS84 table has data.');
    }
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      console.log(`Processing location ${i + 1}/${responses.length}: ID ${locationId} (${location.latitude}, ${location.longitude})`);
    ...
    const locationCount = await client.query('SELECT COUNT(*) FROM sde.openmeteolocations_wgs84');
    ...
    // Don't exit the process for cron jobs, just log the error
  } finally {
    if (client) {
      await client.end();
...
// Function to run once immediately (for testing)
async function runOnce() {
  console.log("Running weather sync once...");
...
function main() {
  // Check if script should run once or as cron job
  ...
  console.log("Starting weather sync service...");
  console.log("Will run every 6 hours (at 00:00, 06:00, 12:00, 18:00)");
  console.log("Press Ctrl+C to stop");
  
  // Schedule to run every 6 hours at the top of the hour
  // Cron format: minute hour day month dayOfWeek
  // 0 */6 * * * means "at minute 0 of every 6th hour"
  cron.schedule('0 */6 * * *', async () => {
    console.log('\n🕐 Cron job triggered - starting weather sync...');
    await runWeatherSync();
  }, {
    scheduled: true,
    timezone: "Africa/Cairo" // Matches your weather timezone
  });
  
  // Also run once immediately when the service starts
  console.log('\n🚀 Running initial weather sync...');
  runWeatherSync();
  
  // Keep the process alive
  process.on('SIGINT', () => {
    console.log('\n👋 Shutting down weather sync service...');
    process.exit(0);
  });
}
